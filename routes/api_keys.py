...
async def generate_api_keys(current_user: dict = Depends(get_current_user)):
...
async def get_my_keys(current_user: dict = Depends(get_current_user)):
    ...
    try:
        api_keys_collection = await get_api_keys_collection()
        user_keys = await APIKeyService.get_user_keys(str(current_user["_id"]), api_keys_collection)
